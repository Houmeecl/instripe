...
function adminGlobalAccounts() {
  ...
  return `<div class="card" style="margin-bottom:20px">
    <div class="card-head"><h3>Cuenta en Chile solo para transferir</h3></div>
    <div class="card-body">
      <p class="funds-note">${escapeAttr(notice)}</p>
      <p class="hint">Es una cuenta en Chile solo para transferir. No recibe pesos, no es tarjeta y no abona a Stripe.</p>
      ${body}
    </div>
  </div>`;
}
...
function globalAccountsBox(company) {
  ...
  const rows = accounts.length
    ...
    : `<div class="account-pair">
        <article>
          <div class="account-head"><h3>Cuenta en Chile solo para transferir</h3></div>
          <p class="hint">Solo para transferir desde Chile. No recibe dinero, no emite tarjeta y no abona a Stripe.</p>
  ...
  const action = accounts.length
    ...
    : `<button class="btn btn-primary" data-global-accounts="${escapeAttr(company.id)}">${icon("plus")} Solicitar cuenta en Chile solo para transferir</button>`;
  return `<section>
